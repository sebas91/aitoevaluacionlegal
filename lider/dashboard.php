...
// Obtener empleados asignados al líder
$empleados = [];
$sql = "SELECT u.* FROM usuarios u 
        JOIN lider_empleado le ON u.id = le.id_empleado 
        WHERE le.id_lider = ? AND u.rol = 'empleado'";

if ($stmt = $conn->prepare($sql)) {
    $stmt->bind_param("i", $liderId);
...
if (count($empleados) > 0 && count($periodos) > 0) {
    ...
    $sql = "SELECT COUNT(*) as total FROM usuarios u 
            JOIN lider_empleado le ON u.id = le.id_empleado 
            WHERE le.id_lider = ? AND u.id IN ($placeholders) 
            AND u.id NOT IN (
                SELECT id_empleado FROM evaluacion_lider 
                WHERE id_lider = ? AND id_periodo IN ($periodoPlaceholders)
            )";
    
    if ($stmt = $conn->prepare($sql)) {
        $types = "i" . str_repeat("i", count($empleadoIds)) . "i" . str_repeat("i", count($periodoIds));
        $params = array_merge([$liderId], $empleadoIds, [$liderId], $periodoIds);
                            ...
                            <div class="row no-gutters align-items-center">
                                <div class="col mr-2">
                                    ...
                                    <div class="h5 mb-0 font-weight-bold text-gray-800">
                                        <?php 
                                        $completadas = count($empleados) * count($periodos) - $evaluacionesPendientes;
                ...
                <div class="col-lg-6">
                    <div class="card mb-4">
                        <div class="card-header bg-primary text-white">
                            <h5 class="mb-0">Mis Empleados</h5>
                        </div>
                        <div class="card-body">
                            <?php if (count($empleados) > 0): ?>
                                <div class="list-group">
                                    <?php foreach (array_slice($empleados, 0, 5) as $empleado): ?>
                                        <a href="evaluar_empleado.php?id=<?php echo $empleado['id']; ?>" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                                ...
                                <?php if (count($empleados) > 5): ?>
                                    <div class="text-center mt-3">
                                        <a href="empleados.php" class="btn btn-outline-primary">Ver todos mis empleados</a>
                                    </div>
                                <?php endif; ?>
                            <?php else: ?>
                                <div class="alert alert-info">
                                    No tienes empleados asignados. Contacta al administrador.
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
